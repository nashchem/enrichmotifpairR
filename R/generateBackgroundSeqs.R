#' Finds GC, length and chromosome matched random genomic sequences
#'
...
generateBackgroundSeqs <- function(input_data, genome_ver = "hg38", xfold = 1,
    ...
    generateBackgroundSeqs_per_chr <- function(input_data_chr, genome_ver = genome_ver, xfold = xfold,
        ...
        assertthat::assert_that(is.data.frame(input_data_chr))
        ...
        # Include only chromosomes from input data
        input_seqnames <- unique(inbed$chr)
        # idx <- match(seqnams, input_seqnames)
        idx <- seqnams %in% input_seqnames
        seqnams <- seqnams[idx]
        chrlens <- chrlens[idx]
        chrpos <- cumsum(as.numeric(chrlens))
        pmax <- max(chrpos)
    ...
    input_data_split <- input_data %>% dplyr::group_by(chr) %>% dplyr::group_split()
    for (k in 1:length(input_data_split)) {
        input_data_chr <- as.data.frame(input_data_split[[k]])
        random_data_chr <- generateBackgroundSeqs_per_chr(input_data_chr, genome_ver = genome_ver, xfold = xfold,
                                                    batchsize = batchsize, nMaxTrials = nMaxTrials)
        random_data <- rbind(random_data, random_data_chr)
    }
    out_data <- random_data %>% GenomicRanges::makeGRangesFromDataFrame(ignore.strand=TRUE,
                                                keep.extra.columns=TRUE)
    return(out_data)
