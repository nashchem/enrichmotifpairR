...
enrich_motifs[enrich_motifs < 1e-100] <- 1e-100
enrich_motifs <- enrich_motifs %>%
    mutate(`-log10(pval_adj)` = -log10(pval_adj))
# The output of the enriched motifs top five 
enrich_motifs %>% head(5) %>% arrange(desc(fold_enrich)) %>% 
    select(motif_name, TF_name, tg_motif_count, bg_motif_count, 
           fold_enrich, `-log10(pval_adj)`) %>%
    kable(., caption="Top 5 enriched motifs")

enrich_motif_pairs <- results$motif_pair_enrich
enrich_motif_pairs[enrich_motif_pairs < 1e-100] <- 1e-100
enrich_motif_pairs <- enrich_motif_pairs %>%
    mutate(`-log10(pval_adj)` = -log10(pval_adj))

# The output of the enriched motif pairs, top five binding partners for **NFY** 
enrich_motif_pairs %>% head(5) %>% arrange(desc(fold_enrich)) %>%
    select(motif_name_1, TF_name_1, motif_name_2, TF_name_2, 
           fold_enrich, `-log10(pval_adj)`) %>% 
...
# keep the order in plotting, by default ggplot2 sorts alphabetically
enrich_motifs_filtered <- enrich_motifs_filtered %>%
    mutate(TF_name = factor(TF_name, levels = rev(unique(TF_name))))
...
**Using another color palette**
```{r H1_ESC_4_3, eval=TRUE, fig.width=4, fig.height=6}
# choose color palette from wesanderson
# Gradient color
col_pal <- colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdYlBu")))(100)
...
enrich_motif_pairs_filtered_mat <- enrich_motif_pairs_filtered %>%
    select(TF_name_1, TF_name_2, `-log10(pval_adj)`) %>%
    tidyr::spread(TF_name_2, `-log10(pval_adj)`) %>% 
    tibble::column_to_rownames(var = "TF_name_1") %>%
    as.matrix() %>%
    tidyr::replace_na(0)

# get the order consistent with "enrich_motifs_filtered"
idx <- match(enrich_motifs_filtered$TF_name, rownames(enrich_motif_pairs_filtered_mat))
enrich_motif_pairs_filtered_mat <- enrich_motif_pairs_filtered_mat[idx, ]
pheatmap::pheatmap(enrich_motif_pairs_filtered_mat, cluster_rows=FALSE, cluster_cols=TRUE, na_col = "grey", angle_col = 45)
